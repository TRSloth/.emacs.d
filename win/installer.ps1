...
if (!(Test-Path "C:\msys64"))
{
    ...
    (New-Object System.Net.WebClient).DownloadFile('https://github.com/msys2/msys2-installer/releases/download/2021-07-25/msys2-base-x86_64-20210725.sfx.exe', 'msys2.exe')
...
#if (Test-Path $wininstall){# if windows doesn't have it installed //also check if installed version is in use
#    $useinstalled = Read-Host 'Emacs detected, overwrite the current config? (Y/N) if no emacs will proceed in the msys2 environment'
#}
